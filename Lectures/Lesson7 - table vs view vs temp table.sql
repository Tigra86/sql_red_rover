--  Lesson # 7 Table vs View vs Temp Table. SQL vs NoSQL
...
-- TABLE                      -- stored set of data; it exists until dropped; 
                              -- table can be created by two ways: 1.define + insert records or 2.from a query
...
-- ---------------------------------------------------
-- VIEW						  -- like "saved query", virtual table; it exists until dropped
...
-- ---------------------------------------------------
-- TMP TABLE					-- stored set of data until session is closed; it exists only in current session
...
Only a few NoSQL datastores are ACID-complaint. 
-- ACID Transactions:
Atomicity: Any transaction that updates multiple rows is treated as a single unit. A successful transaction performs all updates. 
A failed transaction performs none of the updates, i.e., the database is left unchanged.
Consistency: Every transaction brings the database from one valid state to another. It guarantees to maintain all database invariants and constraints.
...
Most NoSQL datastore support BASE model:
-- BASE model
