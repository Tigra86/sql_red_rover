...
-- ETL in Testing
ETL testing is a process that verifies that 
1.the data coming from source systems has been extracted completely, 
2.transferred correctly, and 
3.loaded in the appropriate format
*/
...
ALTER TABLE family.schedule
ADD FOREIGN KEY fk_act(activity_id)
...
-- join all tables together
 select *, 
  case when dayofweek(activity_date) = 1 then 'Sunday'
   ...
   when dayofweek(activity_date) = 7 then 'Saturday' else null end dayofweek
